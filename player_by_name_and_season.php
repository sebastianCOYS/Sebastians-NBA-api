...
if(count($_GET) !== 2){
    $status = ["code" => 400, "message" => "Invalid number of parameters (only accepting 'season' & 'player_name')"];
    echo json_encode(["status" => $status]);
    exit;
}
if(!isset($_GET['player_name']) OR !isset($_GET['season'])) {
    $status = ["code" => 400, "message" => "Missing 'player_name' or 'season' parameter."];
    echo json_encode(["status" => $status]);
    exit;
}
...
try {
    ...
    $stmt = $db->prepare("SELECT * FROM nba_data where player_name = :player_name AND season = :season");
